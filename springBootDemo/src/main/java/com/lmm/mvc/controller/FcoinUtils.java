...
public class FcoinUtils {
    ...
    public static void buy(String symbol,String type,String amount) throws Exception {
        ...
        MediaType t = MediaType.parseMediaType("application/x-www-form-urlencoded; charset=UTF-8");
    ...
    public static double getFtUsdtPrice() throws Exception {
        ...
        BigDecimal b = new BigDecimal((String) jsonArray.get(0)).setScale(3,BigDecimal.ROUND_HALF_UP);
    ...
    //ftusdt
    public void ftusdt(String amount) throws Exception {
        ...
        while (true) {
            ...
            if (ft * marketPrice > usdt&&Math.abs(ft * marketPrice -usdt)<10) {
                //ft太多，需要卖出ft
                BigDecimal b = new BigDecimal((ft * marketPrice - usdt) / marketPrice).setScale(2, BigDecimal.ROUND_HALF_UP);
                sell("ftusdt", "market", b.toString());
                Thread.sleep(300);
            } else if (ft * marketPrice < usdt&&Math.abs(ft * marketPrice -usdt)<10) {
                //ft太少，需要买入ft
                BigDecimal b = new BigDecimal((usdt - ft * marketPrice) / marketPrice).setScale(2, BigDecimal.ROUND_HALF_UP);
                buy("ftusdt", "market", b.toString());
                Thread.sleep(300);
            }
            //买单 卖单

            buy("ftusdt", "market", amount);
            sell("ftusdt", "market", amount);
