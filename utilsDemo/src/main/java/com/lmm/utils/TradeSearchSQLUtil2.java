...
public class TradeSearchSQLUtil2 {
    public static void main(String[] args) throws Exception {
        ...
        //sql = "drop table `INTL_URD_{dbIndex}`.`URD_USER_ORDER_{tbIndex}`;";
        sql = "ALTER TABLE XIMA_URD_{dbIndex}.URD_USER_ORDER_{tbIndex} ADD INDEX `IX_BUYER_ID_CREATE_TIME` (`BUYER_ID`,`CREATE_TIME`);\n" +
                "ALTER TABLE XIMA_URD_{dbIndex}.URD_USER_ORDER_{tbIndex} ADD INDEX `IX_BUYER_ID_TRADE_ORDER_NO` (`BUYER_ID`,`TRADE_ORDER_NO`);";
        int tableNum = 1000;
        int dbNum = 10;
